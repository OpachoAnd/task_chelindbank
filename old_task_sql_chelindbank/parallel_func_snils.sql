create or replace function func_task_chelindbank(snils_number INT, control_number INT)
returns text AS $$
declare 
	sum_number int := 0;
	digit TEXT;
...
begin
	...
	THEN
		return 'Ошибка: Контрольное число неверно';
	ELSE
		return 'Успешно: Контрольное число верно';
	END IF;
